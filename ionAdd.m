...
% charge state. Supports tracer ions with "tracer" suffix.
%
...
%               string scalar or string array. Can include "tracer" suffix
%               to mark the ion as a tracer (e.g., 'H+tracer', 'O+ tracer')
%
...
% plotType = 'ion'
% isTracer = true/false (whether this is a tracer ion)
%
% isotopicCombinations: list of peaks vs. nucleides in the ion and
%               charge state
%
% TRACER SUPPORT:
% Ions can be marked as tracers by appending "tracer" to the ion name:
%   ionAdd(spec, 'H+tracer', ...)    % Hydrogen tracer
%   ionAdd(spec, 'O+ tracer', ...)   % Oxygen tracer (space optional)
%   ionAdd(spec, '2H+tracer', ...)   % Deuterium tracer (specific isotope)
%
% Tracer ions are displayed with diamond markers instead of circles.
% The isTracer flag is stored in h.UserData.isTracer for downstream use.
%
% ToDo:
...
%% check ion name and detect tracer suffix
if isstring(ion)
    ion = char(ion);
end

% Check for tracer suffix
isTracer = false;
tracerPattern = '\s*tracer\s*$';
if ~isempty(regexpi(ion, tracerPattern))
    isTracer = true;
    ion = regexprep(ion, tracerPattern, '', 'ignorecase');
end
...
%% plot
h = ionStemPlot(ax, plotWeight, plotAbundance, ionTypeCS, chargeState, colorScheme, isTracer);

...
meta(contains(meta(:,1),'experiment/specimenTemperatureActual'),2) = num2cell(intRow.TemperatureK);
% start time
...
% end time
endDateTime = intRow.Date + minutes(intRow.RunTimeHrs*60);
...
if ~isnan(intRow.PulseFraction)
    meta(contains(meta(:,1),'experiment/pulseFraction'),2) = num2cell(intRow.PulseFraction);
    meta(contains(meta(:,1),'experiment/pulseType'),2) = {'voltagePulsed'};
end
meta(contains(meta(:,1),'experiment/pulsePeriod'),2) = num2cell((1/intRow.PulseFrequencyHz)*10^9);
meta(contains(meta(:,1),'instrument/flightPathLength'),2) = num2cell(intRow.FlightPathLengthmm);
if ~isnan(intRow.LaserPulseEnergynJ)
    meta(contains(meta(:,1),'experiment/laserPulseEnergy'),2) = num2cell(intRow.LaserPulseEnergynJ);
